...
//        Rectangle robot = new Rectangle(0, 0, 23 * scale, 20 * scale);
    ...
    private void addObstacle(Pane arenaPane, int x, int y, IMAGE_DIRECTION dir) {
        boolean success = arena.addPictureObstacle(x, y, 0, dir);
        if (success) {
    ...
    private void addObstacle(Pane arenaPane, int x, int y, IMAGE_DIRECTION dir, int imageId) {
        boolean success = arena.addPictureObstacle(x, y, imageId, dir);
        if (success) {
            Obstacle obs = new Obstacle(x, y, dir);
            obs.addToPane(arenaPane);
            obsList.add(obs);
